...
export default function Hero() {
  ...
  return (
    <section id="home" className="min-h-screen flex items-center bg-gradient-to-b from-[#87CEEB] via-[#B0DDF5] to-[#CFEFF5] relative overflow-hidden">
      ...
      <div className="absolute -top-32 -left-32 w-[500px] h-[500px]">
        ...
        <div className="absolute inset-8 bg-[#D4AF37]/15 rounded-full blur-[40px] animate-soft-glow" style={{ animationDelay: '0.5s' }}></div>
      ...
      <div className="absolute top-60 right-20 opacity-15 animate-gentle-float" style={{ animationDelay: '3s' }}>
        <div className="relative">
      ...
      {/* Birds */}
      <div className="absolute top-32 left-1/3 opacity-20 animate-gentle-float" style={{ animationDelay: '1s' }}>
        <svg className="w-8 h-4" viewBox="0 0 40 20" fill="none" stroke="#1E4D8F" strokeWidth="1.5">
          <path d="M5 15 Q15 5 25 15" strokeLinecap="round"/>
          <path d="M15 15 Q25 5 35 15" strokeLinecap="round"/>
        </svg>
      </div>
      <div className="absolute top-44 left-1/2 opacity-15 animate-gentle-float" style={{ animationDelay: '2.5s' }}>
        <svg className="w-6 h-3" viewBox="0 0 30 15" fill="none" stroke="#1E4D8F" strokeWidth="1.5">
      ...
      {[...Array(20)].map((_, i) => (
        <div
          ...
          style={{
            ...
            boxShadow: Math.random() > 0.5 ? '0 0 6px rgba(255,215,0,0.4)' : '0 0 4px rgba(255,255,255,0.4)'
          }}
        ></div>
      ))}
        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <div className="text-center md:text-left">
            ...
            <p className="text-xl text-[#1E4D8F]/80 mb-6 font-montserrat font-medium animate-fade-up-delayed">
              Inspiring souls through <span className="text-[#D4AF37] font-bold">spiritual wisdom</span> and <span className="text-[#1E4D8F] font-bold">uplifting words</span>
            </p>
            <p className="text-[#1E4D8F]/60 leading-relaxed max-w-lg mx-auto md:mx-0 font-playfair italic text-base border-l-2 border-[#D4AF37]/50 pl-6 py-2 bg-white/20 backdrop-blur-sm animate-fade-up-delayed">
              "What we read and internalize can shape our attitude. The more uplifting the words, the more one may experience improvement."
            </p>
            <div className="flex flex-wrap gap-3 my-8 justify-center md:justify-start animate-fade-up-delayed-2">
              {authorInfo.roles.map((role, i) => (
                <span key={i} className="px-4 py-2 bg-white/50 backdrop-blur-sm text-[#1E4D8F] text-sm font-montserrat font-medium border border-white/50 hover:border-[#D4AF37]/50 hover:text-[#D4AF37] hover-lift transition-all rounded-full shadow-sm">
                  {role}
                </span>
              ))}
            </div>
            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start animate-fade-up-delayed-2">
              <a href="#about" onClick={(e) => handleScroll(e, '#about')} className="px-8 py-4 bg-gradient-to-r from-[#D4AF37] to-[#C4A030] text-white font-montserrat font-semibold hover:shadow-[0_0_25px_rgba(212,175,55,0.4)] transition-all text-center hover-lift rounded-full shadow-lg">
                About the Author
              </a>
              <a href="#books" onClick={(e) => handleScroll(e, '#books')} className="px-8 py-4 border-2 border-[#D4AF37]/50 text-[#1E4D8F] font-montserrat font-semibold hover:border-[#D4AF37] hover:bg-[#D4AF37]/5 transition-all text-center hover-lift rounded-full bg-white/40 backdrop-blur-sm">
                View Books
              </a>
            </div>
          </div>
          ...
          <div className="relative mx-auto max-w-sm animate-fade-up">
                ...
                <div className="aspect-[3/4] overflow-hidden relative">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.parentElement.classList.add('flex', 'items-center', 'justify-center', 'bg-gradient-to-br', 'from-[#F8F4EA]', 'to-[#CFEFF5]')
            ...
            <div className="absolute top-2 -right-2 w-1.5 h-1.5 bg-[#FFD700] rounded-full animate-gentle-float shadow-[0_0_6px_#FFD700]" style={{ animationDelay: '0.5s' }}></div>
            <div className="absolute bottom-8 -left-2 w-1 h-1 bg-[#FFD700] rounded-full animate-gentle-float shadow-[0_0_4px_#FFD700]" style={{ animationDelay: '1.5s' }}></div>
          </div>
        </div>
      </div>
    </section>
