  ...
  return (
    <section id="books" className="py-24 bg-[#F8F4EA] relative overflow-hidden">
              ...
              <div className="bg-white border border-[#CFEFF5]/30 hover:border-[#D4AF37]/30 transition-all shadow-md hover:shadow-lg hover-lift h-full flex flex-col rounded-2xl overflow-hidden">
                <div className="bg-gradient-to-br from-[#F8F4EA] to-[#CFEFF5]/20 overflow-hidden relative flex items-center justify-center p-4">
                  ...
                  <img
                    ...
                    alt={bookItem.title}
                    className="w-44 h-56 object-contain group-hover:scale-105 transition-transform duration-500"
                    onError={(e) => {
                      e.target.style.display = "none";
                      e.target.parentElement.classList.add("min-h-[220px]");
                      e.target.parentElement.innerHTML = `<div class="text-center p-6"><div class="text-4xl font-playfair font-bold text-[#1E4D8F] mb-2">${bookItem.title
                        .split(" ")
                        .map((w) => w[0])
                        .join("")}</div></div>`;
                    }}
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {bookItem.themes.slice(0, 2).map((theme, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 bg-[#F8F4EA] text-[#63B8E6] text-[9px] font-montserrat font-medium rounded-full border border-[#63B8E6]/10"
                      >
                        {theme}
                      </span>
                    ))}
                  ...
                  <button
                    ...
                    className="w-full py-2 border-2 border-[#D4AF37]/30 text-[#D4AF37] font-montserrat font-semibold text-xs rounded-full hover:bg-[#D4AF37]/5 hover:border-[#D4AF37]/50 transition-all"
                  >
                    View Details
                  </button>
                </div>
      ...
      {selectedBook && (
        ...
        >
          <div
            className="bg-white max-w-6xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-[#CFEFF5]/30 rounded-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between p-6 border-b border-[#CFEFF5]/30 sticky top-0 bg-white z-10">
              <div className="flex items-center gap-3">
                <span className="px-3 py-1 bg-[#D4AF37] text-white text-xs font-montserrat font-semibold rounded-full">
                  {selectedBook.statusText}
                </span>
                {selectedBook.award && (
                  <span className="text-[#D4AF37] text-xs font-montserrat font-semibold">
                    {selectedBook.award}
                  </span>
                )}
              </div>
              <button
                onClick={() => setSelectedBook(null)}
                className="text-[#1E4D8F]/40 hover:text-[#D4AF37] transition-colors"
              >
                <svg
                  className="w-6 h-6"
                  fill="none"
                  ...
                  <div className="bg-[#F8F4EA] rounded-xl p-4 flex items-center justify-center">
                    <img
                      ...
                      onError={(e) => {
                        ...
                        e.target.parentElement.innerHTML = `<div class="text-center p-8"><div class="text-5xl font-playfair font-bold text-[#1E4D8F]">${selectedBook.title
                          .split(" ")
                          .map((w) => w[0])
                          .join("")}</div></div>`;
                      }}
                ...
                <div className="md:col-span-3 flex flex-col justify-center space-y-6">
                  <div>
                    {selectedBook.subtitle && (
                      <p className="text-[#63B8E6] text-xs font-montserrat font-semibold uppercase tracking-wider mb-2">
                        {selectedBook.subtitle}
                      </p>
                    )}
                    <h2 className="text-3xl md:text-4xl font-playfair text-[#1E4D8F] mb-2">
                      {selectedBook.title}
                    </h2>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedBook.themes.map((theme, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 bg-[#F8F4EA] text-[#63B8E6] text-xs font-montserrat font-medium rounded-full border border-[#63B8E6]/10"
                      >
                        {theme}
                      </span>
                    ))}
                  ...
                  {selectedBook.status === "published" &&
                    selectedBook.purchaseLinks && (
                      <div className="pt-4 space-y-3">
                        <p className="text-[#D4AF37] text-sm font-montserrat font-semibold">
                          Available at:
                        </p>
                        <div className="flex flex-wrap gap-3">
                          {selectedBook.purchaseLinks.map((link, i) => (
                            <a
                              ...
                              className="inline-flex items-center gap-2 px-6 py-3 bg-[#D4AF37] text-white font-montserrat font-semibold text-sm rounded-full hover:bg-[#C4A030] transition-all hover-lift"
                            >
                              <svg
                                className="w-4 h-4"
                                fill="none"
